    ...
    def add_bitmasks(self, instances, im_h, im_w):
        for per_im_gt_inst in instances:
            ...
            if isinstance(per_im_gt_inst.get("gt_masks"), PolygonMasks):
                ...
                # avoid error if no instance in the cropped input
                # per_im_gt_inst.gt_bitmasks = torch.stack(per_im_bitmasks, dim=0)
                # per_im_gt_inst.gt_bitmasks_full = torch.stack(per_im_bitmasks_full, dim=0)
                if len(per_im_bitmasks) == 0:
                    per_im_gt_inst.gt_bitmasks = torch.zeros([0, int(im_h//self.mask_out_stride), int(im_w//self.mask_out_stride)], dtype=torch.float, device=self.device)
                    per_im_gt_inst.gt_bitmasks_full = torch.zeros([0, im_h, im_w], dtype=torch.float, device=self.device)
                else:
                    per_im_gt_inst.gt_bitmasks = torch.stack(per_im_bitmasks, dim=0)
                    per_im_gt_inst.gt_bitmasks_full = torch.stack(per_im_bitmasks_full, dim=0)

...
class CondInst_CLIP_CLS(nn.Module):
    def __init__(self, num_classes, temperature, bias_value):
        super(CondInst_CLIP_CLS, self).__init__()

        # hyper-parameters taken from: https://github.com/microsoft/GLIP
        self.logit_scale = nn.Parameter(torch.tensor([0.0]), requires_grad=True)
        self.bias_lang = nn.Parameter(torch.zeros(512), requires_grad=True)
        ...
        # input texts
        if num_classes == 8:
            self.class_labels = ["person", "rider", "car", "truck", "bus", "train", "motorcycle", "bicycle"]
        elif num_classes == 100:
            self.class_labels = ['bed', 'windowpane', 'cabinet', 'person', 'double door', 'table', 'curtain', 'chair', 'car', 'painting', 'sofa', 'shelf', 'mirror', 'armchair', 'seat', 'fence', 'desk', 'closet', 'lamp', 'bathtub', 'railing', 'cushion', 'box', 'column', 'signboard', 'chest of drawers', 'counter', 'sink', 'fireplace', 'refrigerator', 'stairs', 'case', 'pool table', 'pillow', 'screen door', 'bookcase', 'coffee table', 'toilet', 'flower', 'book', 'bench', 'countertop', 'stove', 'palm', 'kitchen island', 'computer', 'swivel chair', 'boat', 'arcade machine', 'bus', 'towel', 'light', 'truck', 'chandelier', 'awning', 'streetlight', 'booth', 'television receiver', 'airplane', 'apparel', 'pole', 'bannister', 'ottoman', 'bottle', 'van', 'ship', 'fountain', 'washer', 'plaything', 'stool', 'barrel', 'basket', 'bag', 'minibike', 'oven', 'ball', 'food', 'step', 'trade name', 'microwave', 'pot', 'animal', 'bicycle', 'dishwasher', 'screen', 'sculpture', 'hood', 'sconce', 'vase', 'traffic light', 'tray', 'ashcan', 'fan', 'plate', 'monitor', 'bulletin board', 'radiator', 'glass', 'clock', 'flag']
        else:
            print('not support.')

        # dim(text_emb): 512 -> 256
        self.text_porj = nn.Linear(512, 256, bias=True)

    def forward(self, x):
        text = clip.tokenize(self.class_labels).to(x.device)
        text_features = self.clip_model.encode_text(text) # (8, 512)
        
        image_features = x
        imshape = image_features.shape
        image_features = image_features.permute(0,2,3,1).reshape(-1, 256)

        # normalized features
        # image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        
        # feature interaction: refer to https://github.com/microsoft/GLIP
        _text_features = text_features / text_features.norm(dim=-1, keepdim=True).float()
        text_features = self.text_porj(_text_features / 0.1) # (8, 256)
        text_feat_bias = torch.matmul(_text_features, self.bias_lang) + self.bias0

        bias = text_feat_bias.unsqueeze(0).repeat(image_features.shape[0], 1)
        cls_logits = (image_features @ text_features.t() / self.logit_scale.exp()) + bias
        cls_logits = cls_logits.view(imshape[0], imshape[2], imshape[3], -1).permute(0,3,1,2)
...
class FCOSHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.temperature = cfg.MODEL.FCOS.TEMP
        self.fpn_strides = cfg.MODEL.FCOS.FPN_STRIDES
...
#         )
        ...
        # modified: replace original cls_logits with CondInst_CLIP_CLS
        self.cls_logits = CondInst_CLIP_CLS(self.num_classes, self.temperature, bias_value)
        ...
        for modules in [
            ...
            self.share_tower, #self.cls_logits.cate_branch[0],

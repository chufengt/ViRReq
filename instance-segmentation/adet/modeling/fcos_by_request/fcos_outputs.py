    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges, num_loc_list):
        ...
        for im_i in range(len(targets)):
            ...
            if self.center_sample:
                ...
                # mask sampling
                bitmasks = targets_per_im.gt_bitmasks_full.bool()
                n_masks = bitmasks.shape[0]
                for mask_i in range(n_masks):
                    xl = xs[is_in_boxes[:, mask_i]].long()
                    yl = ys[is_in_boxes[:, mask_i]].long()
                    is_in_boxes[is_in_boxes[:, mask_i], mask_i] = bitmasks[mask_i, yl, xl]
